...
1. リポジトリにcommitしないファイルは読み手で置き場所を決める
    - ユーザーが参照・保管するもの（レポート・調査結果・生成データ、プロジェクトと無関係のファイル）は現在のプロジェクトroot直下の `ikuwowfiles/` 以下に置き（グローバルgitignore対象）、パスを提示して渡す
    - scratchpadに置くのはAI自身が消費して破棄する中間ファイルだけ（`gh --body-file` 用の一時ファイル等）
    - ユーザーに提示するパスがscratchpad配下になる場合は `ikuwowfiles/` へ置き直す
