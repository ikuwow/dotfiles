...
1. コメントはコードから読み取れない意図の説明のみに限定する
    - 編集履歴の記述（「〜を追加」「〜を削除」「〜に変更」「no longer」「previously」「instead of」等）を書かない
    - plan modeのphaseへの参照、セッション・チャットそのものへの参照を書かない
...
1. テキストの抽出・整形は目的別コマンド（`tr` `cut` `head` `tail` `sort` `uniq` `grep` `jq` `yq` 等）で行い、`sed` `awk` `perl -e` `python -c` `ruby -e` は条件分岐・フィールド演算・複数行の状態保持が要る場合に限定する
