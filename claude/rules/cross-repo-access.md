...
Delegate to the `cross-repo-reader` subagent
(`claude/agents/cross-repo-reader.md`). It owns ghq lookup, optional
fast-forward sync, and file reading via Read/Grep/Glob, and returns a
summary with citations. The agent's frontmatter pins `model: sonnet`,
so the model is enforced regardless of how the parent calls it.
