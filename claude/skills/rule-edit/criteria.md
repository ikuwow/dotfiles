...
- skill / agent定義は、自分の判定に要る規則を自分で持ち、他ファイルを読まずに使える状態にする
  ...
  - 書いてよい参照は、他skill / agentの起動ポインタ（名前のみ）と、そのskillが評価対象を名付ける時のframework名参照（"X defined in `file.md`" 等）に限る
    - 起動ポインタは、名前が解決できなければ起動が失敗して露見するため、黙って壊れる参照に当たらない
    - 起動ポインタが呼び出し先の規則を自分の判定に取り込む場合は、その規則を自分でも持つ（呼び出しが失敗した時に判定が空洞化しないため）
