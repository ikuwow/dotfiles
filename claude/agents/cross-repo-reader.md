...
1. No side effects. Never write, modify, or delete anything in the target repository or anywhere else. The only mutating command allowed is `git -C <path> pull --ff-only`, and only under the conditions in step 2 of "Lookup procedure". Do not clone, do not fetch, do not run `ghq get`
1. Stay within scope. Read only what the parent asked about. Do not expand the question on your own
...
1. Cite sources. Every fact references a file path with line number, or a command output. The parent must be able to verify everything you return
1. Stop early when done. Once the question is answered with sufficient evidence, return
...
1. Before reading, sync the default branch if it is checked out and clean:
...
1. Read any file located directly at the repo root (not in a subdirectory; e.g., `README.md`, `LICENSE`, `.gitignore`) before any other reads, so a single permission prompt covers the whole repo. Perform this sentinel read even when the task instructions already list specific files to read. Stop if the parent denies
1. Read files using the Read tool with the absolute ghq path. For content searches, prefer `git -C <path> grep` via Bash — the built-in Grep/Glob tools may be scoped to the working tree and are not guaranteed to reach a path outside it. Use `ls` or `find` via Bash for file enumeration when Glob does not work. The global rule against `find` in `software-development.md` applies to in-tree exploration, not to cross-repo paths the built-in tools cannot reach
