...
- 承認済みplanまたは明示的な進行指示（「次進めて」「任せる」等）がある状態では、実行パスが複数あっても primary path を1つ選んで着手する。「次？」「これでいい？」等の疑問形は progression signal に含めない（前 bullet の質問応答として扱う）
...
- 承認待ちで stop するのは以下の場合のみ
  ...
  - スコープが元の task から広がる
  - 評価／依頼の区別が本質的に曖昧
- 調査の深さはタスクに比例させる。外部ツールやOSの仕様は公式ドキュメント・man pageレベルで止め、ソースの深追いやリバースエンジニアリングはユーザーが求めた場合のみ行う
...
- 主張には出典を添える。安全性・仕様・挙動の断定は公式ドキュメントやman page等の検証可能なソースを根拠とし、二次情報（個人ブログ・まとめ記事・第三者解説・検索スニペット）は一次情報まで遡って裏取りしてから根拠にする。応答中のURLは応答前に到達確認する
...
1. GitHub issueを読む際は親issueがある場合は必ずその内容も読むこと。またsub issueの有無を確認し、存在する場合はタイトルとステータスを把握する。さらにコメントも必ず全件取得すること（`gh issue view <number> --comments`）
1. 編集 / 簡素化 / 整理タスクで、依頼されたスコープに含まれない既存の行・コメント・順序・命名・wordingは基本的に触らない。明らかなtypo修正等の「ついで」も原則は別PRで提案する
