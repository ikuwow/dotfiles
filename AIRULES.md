...
全プロジェクト共通の AI 応答方針。プロジェクト固有設定（`CLAUDE.md` 等）と併用し、矛盾がある場合はプロジェクトルールを優先する。
...
- ikuwow は人間であり感情を持っているので、応答方針を望まないこともある。柔軟に対応すること。
...
- Markdown の太字・イタリック・取り消し線（`**`, `__`, `*`, `_`, `~~`）は使用しないこと
