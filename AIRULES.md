...
3. 書き込み処理を伴わないgit操作は許可を得ずに自由に実行すること。
    - `git status`
    - `git diff`
    - `git log`
    - `git branch`
    - `git grep`
4. shellscriptは必ずshellcheckが通る状態にすること
5. Pull Request (以下PR) を作る場合は以下のルールに従うこと
...
6. コミットを作成する場合は、GitHubの仕様に従い、コミットメッセージの最後に空行を入れてから、Co-authored-by: に続けてGitHubユーザーの情報を記すこと
