...
1. gcloud / bq / gsutilを実行する際は必ず `CLOUDSDK_CORE_PROJECT=<project> <gcloud|bq|gsutil> ...` の形式で環境変数をコマンドの先頭に付与する。使用するprojectは必ずユーザーに確認してから実行すること。project切替は `gps` コマンドを使う
